...
<div class="flex flex-col gap-2 overflow-y-auto pb-6">
    ...
    <a href="{{ route('admin.students.index') }}" wire:navigate
        class="flex items-center rounded-lg gap-3 px-4 py-3 text-sm font-medium transition-all duration-200
        {{ request()->routeIs('students*') || request()->routeIs('admin.students*')
            ? 'bg-gradient-to-r from-secondary-400 to-secondary-300 text-white shadow-md'
            : 'text-gray-600 hover:bg-gradient-to-r hover:from-secondary-400/10 hover:to-secondary-300/10 hover:text-secondary-400' }}
        group">
        <div
            class="p-2 rounded-lg transition-colors
            {{ request()->routeIs('students*') || request()->routeIs('admin.students*') ? 'bg-white/20' : 'bg-gray-100 group-hover:bg-secondary-400/20' }}">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
    ...
    <a href="{{ route('admin.teachers.index') }}" wire:navigate
        class="flex items-center rounded-lg gap-3 px-4 py-3 text-sm font-medium transition-all duration-200
        {{ request()->routeIs('teachers*') || request()->routeIs('admin.teachers*')
            ? 'bg-gradient-to-r from-secondary-400 to-secondary-300 text-white shadow-md'
            : 'text-gray-600 hover:bg-gradient-to-r hover:from-secondary-400/10 hover:to-secondary-300/10 hover:text-secondary-400' }}
        group">
        <div
            class="p-2 rounded-lg transition-colors
            {{ request()->routeIs('teachers*') || request()->routeIs('admin.teachers*') ? 'bg-white/20' : 'bg-gray-100 group-hover:bg-secondary-400/20' }}">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
        ...
        <span class="{{ request()->routeIs('teachers*') || request()->routeIs('admin.teachers*') ? 'font-semibold' : 'font-medium' }}">Teachers</span>
        @if (request()->routeIs('teachers*') || request()->routeIs('admin.teachers*'))
            <span class="sr-only">active</span>
        @endif
    </a>
