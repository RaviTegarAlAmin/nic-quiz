    ...
    <x-card x-data="{
        ...
        <div class="flex gap-2 mb-4">
            @foreach ($schedules as $grade => $classrooms)
                <button
                    x-on:click="activeGrade = '{{ $grade }}'; activeClass = '{{ array_key_first($classrooms) }}'"
        ...
        @foreach ($schedules as $grade => $classrooms)
            <div x-show="activeGrade === '{{ $grade }}'" class="flex items-center gap-2 mb-5">
                <button
        ...
        {{-- Timeline --}}
        @foreach ($schedules as $grade => $classrooms)
            <div x-show="activeGrade === '{{ $grade }}'">
                @foreach ($classrooms as $className => $days)
                    <div x-show="activeClass === '{{ $className }}'" class="space-y-3">
                                ...
                                <div class="flex gap-2 flex-wrap">
                                    @foreach ($periods as $code => $period)
                                        @if ($period['course'] === null)
                                            <div class="border rounded-lg px-2.5 py-1.5 text-xs bg-gray-50 text-gray-800 border-gray-200">
                                                <p class="font-medium">{{ $period['type'] == 'lecture' ? 'TBD' : 'Istirahat' }}</p>
                                                <p class="text-xs opacity-70">{{ \Carbon\Carbon::parse($period['start_at'])->format('H.i') }}
                                                </p>
                                            </div>
                                        @else
                                            @php
                                                $colors = [
                                                    'MTK' => 'bg-secondary-50 text-secondary-800 border-secondary-200',
                                                    'IPA' => 'bg-teal-50 text-teal-800 border-teal-200',
                                                    'IPS' => 'bg-danger-50 text-danger-800 border-danger-200',
                                                    'IND' => 'bg-warning-50 text-warning-800 border-warning-200',
                                                ...
                                                $endTime = \Carbon\Carbon::parse($period['start_at'])
                                                    ...
                                                    ->format('H.i');
                                            @endphp
                                            <div class="border rounded-lg px-2.5 py-1.5 text-xs {{ $color }}">
                                                <p class="font-medium">{{ $period['course_code'] }}</p>
                                                <p class="text-xs opacity-70">{{ $startTime }}–{{ $endTime }}
                                                </p>
                                            </div>
                                        @endif
                                    @endforeach
